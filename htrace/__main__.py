...
def cbLinkFollow(response, *args, **kwargs):
    ...
    if session._extra['link_type'] is not None:
        if response.headers.get('Content-Type', '').find(session._extra['link_type']) >= 0:
            ...
            if _profile.find(session._extra['link_profile']) >= 0:
                L.info(f"Match linked type {R}{session._extra['link_type']}{W} and profile {R}{session._extra['link_profile']}{W}")
                return
    ...
    for lh in lhs:
        if lh['type'] == session._extra['link_type']:
            if session._extra['link_profile'] is None or lh.get('profile', '') == session._extra['link_profile']:
